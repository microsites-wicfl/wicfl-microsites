...
Esta es la guía, no el material de referencia. Te dice qué es este repo, qué tocas tú, qué no
tocas nunca, y cómo se ve una semana real. Cuando necesites los campos exactos del schema, las
...
generador convierte eso en el sitio en vivo. Tú nunca tocas HTML, CSS, ni el código del
template. Si en algún momento sientes que quieres hacerlo, parate y avisa. Querer editar un
...
**Tú escribes el contenido. Nada se publica desde un brief generado por IA.** Tu documento
...
Avisa. Un campo de config que falta, un layout del template que no puede hacer lo que una página
...
1. Define la página o el sitio con la skill `microsite-brief` si estás arrancando de cero —
   produce el config, el mapa de páginas y el plan de diferenciación antes de generar nada.
2. Escribe el markdown, página por página. Conforme cada página queda escrita, corre la skill
...
3. Sube una rama (o, para una edición simple, usa el formulario — ver Part 2a). Un pull request
   te da automáticamente una URL de preview en vivo, publicada como comentario y actualizada en
   cada push. No necesitas tener un servidor local corriendo para ver tu trabajo renderizado; eso
   también existe (`docs/SETUP.md`) pero es una comodidad, no un requisito.
4. Antes de pedir que se mergee, corre el checklist de autorevisión de `docs/CONTENT_STANDARDS.md`
   y el checklist completo de lanzamiento de `docs/QA_CHECKLIST.md`. Ambos son tuyos para correr;
   por ahora nadie más revisa antes que tú (ver la nota sobre esto más abajo).
5. Mergea. Una vez que el deploy de producción real esté activado para un sitio, eso es lo que lo
   publica en vivo.

### Quién revisa tu contenido

Por ahora, nadie más. El checklist de autorevisión de `docs/CONTENT_STANDARDS.md` existe porque
hoy no hay una segunda persona en el equipo dedicada a revisar. Puedes preguntarle a Kevin de
...
El Part 1 te dijo qué tocas y por qué. Esta parte es el "cómo", en dos caminos:
...
Si lo que necesitas hacer hoy es "cambiar un párrafo de una página que ya existe" o "actualizar
el título de SEO", ve directo al Part 2a. Si necesitas crear una página que no existe todavía,
ve al Part 2b.
...
repo, pensado exactamente para que tú publiques un borrador sin tener que entender rama, commit,
ni pull request. Por dentro sigue creando una rama y abriendo un pull request igual que el camino
de GitHub — nada más lo hace por ti, con botones en español conceptual en vez de vocabulario de
git.

1. Entra a `https://wicfl-content-form-w111.wicfl-microsites.workers.dev`.
2. Inicia sesión con la contraseña compartida (la misma que usa el equipo para este formulario —
   pídesela a Vic si no la tienes, vive en el vault del proyecto, nunca por chat).
3. Elige el sitio en el selector, y después la página que quieres editar.
...
5. Edita lo que necesites y haz clic en **"Publish draft."** Esto crea el pull request por ti,
   automáticamente.
6. El formulario te muestra un link al pull request creado (algo como `.../pull/12`). Abrilo:
   ...
   rama y el PR a mano.
7. **El formulario nunca mergea nada.** Alguien (tú mismo, o Vic si prefieres una segunda mirada)
...
Vas a ver estas palabras en GitHub mismo y en el resto de esta guía. No necesitas entender git
...
- **Rama (branch)** — una copia paralela y separada de los archivos donde puedes hacer cambios sin
  tocar `main` todavía. Cuando editas un archivo en el sitio web de GitHub, GitHub crea una de
...
1. Ve a `github.com/microsites-wicfl/wicfl-microsites` e inicia sesión.
2. Haz clic en `sites/<tu-sitio>/content/`, y después en el archivo `.md` que quieres cambiar
   (por ejemplo `flood-coverage.md`).
3. Haz clic en el ícono de lápiz arriba a la derecha de la vista del archivo ("Edit this
   file").
4. Edita el texto. Es markdown simple: `## Encabezado` para un título, una línea en blanco entre
   ...
   más abajo.
5. Baja hasta el final. Debajo de "Commit changes," escribe un mensaje corto describiendo qué
...
6. Asegúrate de que esté seleccionada la segunda opción: **"Create a new branch for this commit
...
7. Haz clic en **"Propose changes."**
8. Llegas a una página de "comparing changes." La caja de título viene prellenada desde tu
   mensaje de commit — edítala si quieres algo más claro. Haz clic en **"Create pull request."**
...
9. Ya estás en la página del pull request. Espera un minuto o dos, recargando si hace falta,
...
10. Cuando `Preview deploy` termine, aparece un comentario en el PR de GitHub Actions con un link
    parecido a `https://wicfl-prNN-<slug>.wicfl-microsites.workers.dev`. Haz clic ahí: es tu
...
11. Mira la página real. Después corre el checklist de autorevisión de `docs/CONTENT_STANDARDS.md`,
    y `docs/QA_CHECKLIST.md` si esta es una pasada de listo-para-lanzar.
12. Si ambos checks están verdes y el preview se ve bien, haz clic en el botón verde **"Merge
    pull request,"** y después en el botón de confirmación que aparece debajo. Después, GitHub
    ofrece un botón **"Delete branch"** — haz clic ahí; la rama era solo andamiaje para el PR.
...
1. Ve a `sites/<tu-sitio>/content/` en el repo.
...
2. Haz clic en el desplegable **"Add file"** cerca de arriba a la derecha del listado, y
...
3. En el campo **"Name your file..."**, escribe un nombre de archivo terminado en `.md`:
...
4. En el editor, lo primero que tiene que aparecer en el archivo es un bloque de frontmatter:
   ...
   - `description` — opcional, pero escribe uno igual; es lo que aparece en los resultados de
   ...
   - `pageType` — obligatorio, y tiene que ser exactamente una de tres palabras: `home`,
     ...
     portada — no crees una segunda). Para todo lo demás usa `content`, salvo que la página sea
     ...
     genuinamente no estás seguro entre `content`/`coverage`, usa `content` y pregúntale a Vic —
...
5. Baja y commiteá exactamente como en los pasos 5–13 de arriba: rama nueva, pull request,
   esperar los checks, preview, revisión, merge.

**Lo único que te va a morder si te lo saltas:** este sitio no tiene ningún menú de navegación
todavía — sin links en el header, sin mapa en el footer, nada que liste las páginas
automáticamente. Una página que publicas así queda en vivo en su URL, pero inalcanzable para
cualquiera que navegue el sitio a menos que otra página la enlace. Así que antes de mergear una
página nueva, edita al menos una página existente (la home suele ser la elegida) y agregale un
...
El contrato de frontmatter de arriba son solo tres líneas. Acá tienes un ejemplo completo de cómo
...
Fíjate qué está haciendo cada parte, y por qué está ahí:
...
- **Detalle real y local** ("St. Lucie River," "Martin County," "dock," "screened lanai") — esto
  ...
  `docs/CONTENT_STANDARDS.md`. Si intercambias la ciudad, la mayor parte de este párrafo deja de
...
1. **Escribe el frontmatter y el contenido de cada página en otro lado primero** — un editor de
...
2. Ve a `sites/<tu-sitio>/content/`, haz clic en **"Add file" → "Create new file,"** igual que
   los pasos 1-2 de "Crear una página nueva" de arriba, pero solo para tu **primera** página.
3. Nombra el archivo, pega su frontmatter y contenido, igual que los pasos 3-4 de arriba.
4. Baja hasta **"Commit changes..."**. Esta vez, selecciona **"Create a new branch for this
   commit and start a pull request,"** y antes de hacer clic, reemplazá el nombre de rama
   autogenerado (algo como `<tú>-patch-2`) por algo corto y legible, por ejemplo
   `site1-real-content`. Haz clic en el botón que crea la rama.
5. Llegas a la misma página "Open a pull request" que antes. **Todavía no crees el pull
   request.** Todas las demás páginas todavía tienen que subirse a esta misma rama primero.
6. Para cada página restante, ve directo a esta dirección, con el nombre de tu rama y el slug
   ...
   en vez de a `main` — revisa la etiqueta chica junto al campo de nombre de archivo, debería
   decir **"in \<tu-nombre-de-rama>,"** no "in main."
7. Nombra el archivo, pega su contenido, baja hasta **"Commit changes..."**. Como la rama ya
   existe, GitHub ahora por defecto va a mostrar **"Commit directly to the
   \<tu-nombre-de-rama> branch."** Eso es lo que quieres esta vez — déjalo así, no toques los
   botones de radio, solo haz clic en **"Commit changes."**
8. Repite los pasos 6-7 para cada página restante.
9. Una vez que cada página está commiteada, ve a:
   ...
   content, 8 pages"), y una descripción listando cada página que incluye. Haz clic en
   **"Create pull request."**
10. Espera los checks, igual que el paso 9 de "Editar una página que ya existe" de arriba — esta
...
11. Haz clic en el link de preview que comenta el bot. **Navegá varias páginas, no mires solo
...
12. Corre el checklist de autorevisión de `docs/CONTENT_STANDARDS.md` en cada página, no una sola
    vez para todo el lote.
13. Si todo está en orden, mergea, confirma, y borra la rama, igual que los pasos 12-13 de
    arriba.
...
que no es una de las tres palabras permitidas) o un desliz de sintaxis de markdown. Haz clic en
...
|---|---|
| `docs/ARCHITECTURE.md` | Quieres las decisiones y el razonamiento detrás de ellas, una vez, antes del handoff |
| `docs/SITE_CONFIG_SCHEMA.md` | Estás llenando o depurando un `site.config.json` |
| `docs/SITE_CONFIG_SCHEMA.md`, decisión de diseño 7 | Estás agregando un logo — pon el archivo en `sites/<slug>/public/` y apunta `brand.logo` a él |
...
| `docs/QA_CHECKLIST.md` | Crees que una página o un sitio está listo para publicarse |
| `docs/SETUP.md` | Quieres un servidor local corriendo, o estás depurando el pipeline de deploy en sí |
| `docs/GATE_B_MODEL.md` | Quieres entender qué significa "llamada calificada" y por qué la vara de Gate B está armada así |
| `BACKLOG.md` | Quieres saber qué está construido, qué deliberadamente no está construido todavía, y por qué |

Y dos skills que corres tú mismo, no solo lees:
...
- **`differentiation-audit`** — la prueba de intercambio, corrida como skill en vez de como
  ejercicio mental. Correla por página, mientras escribes.
...
1. **Ningún sitio se construye a mano.** Se genera desde su config. Si el generador no puede
   producir algo que necesitas, eso es un problema del generador, no una razón para editar el
   resultado a mano.
2. **La prueba de intercambio es ley.** Hay un gate de CI para esto una vez que exista el Sitio
   #2 contra el cual comparar (W-027), pero durante el Sitio #1 lo único que la hace cumplir eres
   tú, corriendo la skill por página.
