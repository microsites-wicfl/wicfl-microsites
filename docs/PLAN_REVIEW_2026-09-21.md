...
### Proposal 1 — Gate B cannot be answered organically in 120 days; add a smoke test and leading indicators, and decide cadence rather than a site count
...
- **Add a small paid smoke test, on Site #1 only.** Insurance is the most expensive vertical in Google Ads: 2026 benchmarks put homeowners keywords at $30–50 per click, 2.5–4% conversion, $80–140 per lead. That means a real conversion-rate measurement costs thousands, and this review does not propose one. What it proposes is a smoke test: $200–300 total over three to four weeks in October–November, then off, on narrow long-tail and geographic keywords that match the site's niche exactly ("waterfront home insurance Stuart FL", "coastal home insurance Martin County"), on Microsoft Ads as well as Google since Microsoft's clicks are cheaper and skew older, which fits the Treasure Coast homeowner. That buys roughly 20–30 real buying-intent visits. The question it answers is not "what is the conversion rate" but "does a real stranger who lands on this page call or fill the form": one or two leads out of 25 clicks says the page works; zero out of 30 is a warning. Whatever the result, it exercises the whole funnel with real traffic (form to GoHighLevel, call to GoTo recording) before the first organic lead arrives. Two things to be precise about. First, **this is a one-time diagnostic, not part of the model.** At a hundred sites the model has to work organically or it is not a model: a site that only produces calls with ads behind it adds nothing over pointing the same ad spend at `wicfl.com`. Second, **paid ads do not move organic rankings.** Google has said repeatedly that the two systems are separate. The ads buy evidence and leads, not positions. Because the paid test is small, the weight of the March decision falls on the organic leading indicators below, not on the ads.
...
- **Make Gate B a decision about cadence, not a site count, and set the cadence from content capacity.** The current plan commits nothing beyond Site #3 until Gate B, then says "scale to 20" from March with no rate. The plan also assumes 15 to 25 pages per site. Those two facts do not fit together with one writer, and the plan does not do the arithmetic. Here it is. Pavel wrote Site #1's eight pages between 31 Aug and 9 Sep while still ramping; call it one real, locally-grounded insurance page per day, which is fast. Each site also needs niche validation, config, QA, launch, tracking number, Search Console and reporting, roughly two days, plus ongoing SEO on the sites already live. That leaves about 12 to 14 writing days a month.

  | Pages per site | Sites per month, Pavel alone | 20 sites takes | 100 sites takes |
  |---|---|---|---|
  | 20 (what the plan assumes) | ~0.6 | ~3 years | ~14 years |
  | 8 (Site #1 as built) | ~1.5 | ~13 months | ~5.5 years |
  | 6 | ~2 | ~10 months | ~4 years |
  | 4–5 | ~3 | ~7 months | ~3 years |

  The proposal: **six pages per site, two new sites a month**, starting once Gate A passes on 13 Nov rather than waiting for March. Every page still has to pass the swap test; six is the size at which Pavel can hold that standard at this pace. Gate B in March becomes a continue-or-stop decision on that cadence, taken on the smoke test plus the leading indicators. This means arriving in March with about eight sites and no commercial evidence yet; the marginal cost of each is about $26 a year plus Pavel's time, and if the content is real the doorway risk is per page, not per site.

  The table also says something the plan should say out loud: with Pavel as the only writer, which is the staffing decision as it stands, a hundred sites is a four-year horizon at this size, not a six-month one. The factory can generate a hundred sites in an afternoon; the content cannot be generated, and generating it is exactly what Google's scaled-content policy is written against. Twenty sites by late 2027 is the realistic version of the plan's Phase 10. Two a month is the starting pace, not a ceiling: once the first sites show organic results and Pavel has the routine down, the cadence can grow, and the factory is built for that. What grows it is either more writing capacity or smaller sites, and that is a decision to take with data in hand, not now.

**What it asks of Kevin.** Approve a $200–300 one-time ad budget for October–November; allow links from Walker's existing sites to the microsites; agree to six-page sites at two a month after Gate A instead of waiting for March, as a starting pace with room to grow, with March deciding whether it continues; and read the hundred-site target as a multi-year horizon with Pavel as the only writer.
...
|---|---|---|
| 1 | Gate B can't be answered organically in 120 days: one-time paid smoke test on Site #1, real organic accelerators, leading indicators; six-page sites at two a month after Gate A, Gate B decides whether that continues | Approve $200–300 one-time ad budget for Oct–Nov; allow links from Walker's sites; agree to 6 pages/site and 2 sites/month after Gate A; read 100 sites as a multi-year horizon with one writer |
