...
| W-107 | **Renderer de markdown real, con content collections de Astro** | Vic | **Código en `main` desde el 2026-09-02 (`478d7b2`), SIN REPORTE.** El prompt 007 se ejecutó dos veces: la primera corrida dejó código y reporte, ambos se revirtieron, y la segunda corrida reaplicó código distinto sin volver a escribir el reporte. En el árbol hay content collections con `glob` loader apuntando al contenido externo del sitio y frontmatter validado con zod. **No se puede cerrar sin reporte**: falta el razonamiento de diseño, que es lo único que el diff no contiene. | **Abierto el 2026-09-02 al revisar W-026. Fecha límite: 18 de septiembre, el handoff.** El renderer propio de `site-data.mjs` solo maneja párrafos, `h2`, `h3` y saltos de línea: no hay enlaces, listas, negritas, imágenes ni tablas. Dos razones para no dejarlo: `docs/ARCHITECTURE.md` justifica haber elegido Astro citando sus content collections, así que estamos pagando el framework sin usar la parte que lo justificaba; y Pavel escribe contenido desde el 21 de septiembre, donde una página de coberturas es una lista con enlaces. Si escribe una lista y sale un párrafo con guiones, la respuesta va a ser pedirle a Vic que toque el template, que es la dependencia que el handoff existe para cortar |
| W-021 | Template base de Astro: layouts, componentes, design system | Vic | **Mismo caso que W-107: código en `main` sin reporte.** Las dos correcciones obligatorias sí se aplicaron y están verificadas en el árbol: `displayPhone` se muestra con `href="tel:"` sobre `trackingPhone`, y la dirección se renderiza. | **Dos correcciones obligatorias del 2026-09-02, no son mejoras:** (1) el pie de página muestra `trackingPhone` en E.164, o sea que un visitante ve `+17720000000`; el schema tiene `displayPhone` justo para eso y no se usa en ninguna parte. Va `displayPhone` como texto visible y `trackingPhone` en el `href="tel:"`. (2) `contact.address` no se renderiza, y la consistencia de NAP es regla permanente en `docs/CONTENT_STANDARDS.md`: nombre, dirección y teléfono tienen que coincidir con el Google Business Profile o se suprimen rankings locales. Hoy la plantilla publica dos de los tres y uno mal formateado. **Nota del 31-ago:** `npm run check` quedó reducido a validar configs porque `astro check` se colgaba en local. CI no se ve afectado, pero el comando local ya no detecta errores de tipo antes de construir. Devolverlo al cerrar este item |
