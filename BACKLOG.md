...
| W-011 | Registrar dominios de sitios #1 y #2 vía Cloudflare Registrar | Vic | Depende de W-007, que ahora depende de W-016. **Nota 2026-09-03:** Kevin ya compró/posee `StuartHomeownersInsurance.com`, confirmado registrado en **GoDaddy**, no en Cloudflare Registrar como fija el plan original (`docs/ARCHITECTURE.md`). **Avance 2026-09-03:** Vic corrió "Add a Site" en Cloudflare para el dominio. Zona creada, registros DNS importados automáticamente (2 A hacia el parking de GoDaddy, `www` CNAME, `_domainconnect` CNAME, `_dmarc` TXT — se dejan tal cual por ahora). Cloudflare asignó los nameservers `ashley.ns.cloudflare.com` y `ram.ns.cloudflare.com` para reemplazar los de GoDaddy (`ns77.domaincontrol.com`, `ns78.domaincontrol.com`). Falta: (1) enviar esos dos nameservers a Kevin para que los cambie en GoDaddy, (2) esperar propagación, (3) una vez activo, reemplazar los 2 registros A del parking por el Worker real y borrar el `_domainconnect` CNAME que ya no sirve. Pendiente para la próxima vez: comprar dominios futuros directo en Cloudflare Registrar para saltarse este paso **Avance 2026-09-04:** Kevin le dio a Vic acceso de delegado a su cuenta de GoDaddy (vía Delegate Access, sin compartir contraseña), así que de aquí en adelante Vic puede cambiar nameservers y gestionar DNS directo, sin pedírselo a Kevin cada vez |
