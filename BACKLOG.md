...
| W-012 | Crear org de GitHub y monorepo, configurar accesos | Vic | **Avance 2026-09-02:** `main` está remoto y es la rama por defecto; el dispatch manual y un PR desechable probaron que Actions y el trigger `pull_request` sí corren. Ambos CI fallaron en `npm run check` porque AJV recibe demasiados argumentos bajo Bash; W-012 no se cierra hasta CI verde. Preview se disparó en el PR y su job quedó omitido por guard; deploy no se disparó. **Decisión de identidad:** `microsites@wicfl.com` es Owner de la org; las personas entran con su cuenta propia, nunca con la del buzón. `vitoriomanzarek` tiene escritura. Prompt: `prompts/2026-09-02_004_verificar-triggers-ci.md`; reporte: `reports/2026-09-02_004_verificar-triggers-ci.md`. |
| W-013 | Configurar zonas, DNS y SSL | Vic | Depende de W-011 |
| W-106 | **`.gitattributes` que normalice finales de línea a LF** | Vic | **Abierto el 2026-09-02.** El repo no normaliza y no hay `core.autocrlf`, así que una herramienta que reescriba un archivo en Windows lo deja completo como modificado. No es cosmético aquí: la regla 11 de `CLAUDE.md` dice que la revisión se hace contra el diff, y un diff con 250 líneas idénticas borradas y vueltas a agregar esconde el cambio real en el ruido. Se hace ahora, antes de que haya más archivos |
| W-014 | Levantar pipeline de deploy con Workers Static Assets + GitHub Actions | Vic | **Avance 2026-08-31:** CI, deploy y preview workflows escritos en `1c37278`; deploy y preview siguen desactivados hasta W-092/W-010. Prompt: `prompts/2026-08-28_002_monorepo-y-pipeline.md`; reporte: `reports/2026-08-28_002_monorepo-y-pipeline.md`. Incluye los preview deploys por rama de W-098. Montarlos después cuesta el doble. **Bug encontrado el 2026-09-02 al ver correr CI por primera vez:** `npm run check` falla en Linux con `too many arguments`, porque el script pasa `-d packages/config-schema/examples/*.json` sin comillas y bash expande el glob antes de que AJV lo vea. En Windows no se notaba porque la shell no expande ese patrón. El comando de validación diario del proyecto estaba roto en la única plataforma donde siempre va a correr. Se corrige en el prompt 005 |
